...
"""
Here is the method for extracting security patterns of Integer overflow vulnerability.
"""

# 存储pattern
def store_pattern(data: dict):
    info_json = json.dumps(data)
    with open("../pattern_feature/arithmetic.json", "a+") as f:
...
def split_function(filepath):
    ...
    for line in lines:
        ...
        if len(text) > 0 and text != "\n":
            if text.split()[0] == "function" or text.split()[0] == "constructor": # 如果text以function或constructor开头，则将该函数加入
                function_list.append([text])
                flag += 1
            elif len(function_list) > 0 and ("function" or "constructor" in function_list[flag][0]): # 将函数内部的语句加入
...
def extract_pattern(filepath):
    ...
    # Store functions other than W functions (with .call.value)还应该添加上transfer 存储调用过.call.value的函数
    ...
    ################   pattern 2  是否在调用call.value后才修改余额 #######################
    ...
    ################   pattern 3 转账前是否有检查用于余额；这里的需要更改，应该设置为检查到param后判断转账前有没有判断余额 #######################
    for i in range(len(callValueList)):
        CallValueFlag2 = 0
        param = None
...
if __name__ == "__main__":
    ...
    label = None
    inputFileDir = "../data/dataset/arithmetic/"
    outputfeatureDir = "../pattern_feature/feature_zeropadding/arithmetic/"
    ...
    for file in dirs:
        ...
        # np.pad填充数组，(0,60)表示在前面填充0个，后面填充60个0。'constant'表示填充模式为填充整数，默认0
